...
BEGIN
  -- Marcação temporária (válida só dentro da transação de create_order):
  -- permite o UPDATE interno e legítimo que preenche subtotal/total.
...
BEGIN
  ...
  -- Marca este UPDATE interno como confiável (válido só nesta transação).
...
GRANT EXECUTE ON FUNCTION public.create_order(uuid, jsonb, payment_method, numeric, text, jsonb) TO authenticated;
